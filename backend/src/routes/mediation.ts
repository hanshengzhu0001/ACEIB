...
router.post('/tickets', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Create ticket
    const ticket = new MediationTicket({
      issueType,
      title,
      description,
      reporter: reporterId,
    ...
    logger.info(`Mediation ticket created: ${ticket.ticketNumber} - ${title}`);
...
router.get('/tickets', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const tickets = await MediationTicket.find(query)
    ...
    const count = await MediationTicket.countDocuments(query);
...
router.get('/tickets/:id', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const ticket = await MediationTicket.findById(req.params.id)
      .populate('reporter', 'firstName lastName email')
      .populate('reported', 'firstName lastName email')
      .populate('assignedTo', 'firstName lastName email')
    ...
    // Check permissions
    const isInvolved = [ticket.reporter, ticket.reported, ticket.assignedTo].some(id =>
...
router.put('/tickets/:id/status', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// @access  Private/Admin
router.put('/tickets/:id/assign', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const userId = (req as any).user.userId;
    const userRole = (req as any).user.role;
    ...
    const ticket = await MediationTicket.findByIdAndUpdate(
      req.params.id,
      {
        assignedTo: userId,
        status: 'in-review',
...
// @access  Private/Admin
router.post('/tickets/:id/notes', async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { note } = req.body;
    const userId = (req as any).user.userId;
    const userRole = (req as any).user.role;
...
// @access  Private/Admin
router.put('/tickets/:id/resolve', async (req: Request, res: Response, next: NextFunction) => {
  try {
...
router.get('/stats', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [
      ...
      MediationTicket.countDocuments(),
      MediationTicket.countDocuments({ status: 'open' }),
      MediationTicket.countDocuments({ status: 'resolved' }),
      MediationTicket.aggregate([
    ...
    const issueTypeStats = await MediationTicket.aggregate([
